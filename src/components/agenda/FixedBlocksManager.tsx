    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-xl font-semibold text-foreground">Blocos Fixos</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-7 gap-4">
                {blocksByDay.map(day => (
                    <div
                        key={day.value}
                        className="bg-foreground/5 rounded-2xl p-4 border border-card-border/50"
                    >
                        <h3 className="text-sm font-medium text-foreground/80 mb-3">{day.full}</h3>

                        <div className="space-y-2 min-h-[100px]">
                            {day.blocks.length === 0 ? (
                                <p className="text-xs text-muted/70 text-center py-4">
                                    Sem blocos
                                </p>
                            ) : (
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className="flex items-start gap-2">
                                                <Icon className="w-4 h-4 text-foreground/80 mt-0.5" />
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <div className="relative">
                                            <Clock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted/70" />
                                            <input
                                                type="time"
                                                value={formData.start_time}
                                                onChange={(e) => setFormData(prev => ({ ...prev, start_time: e.target.value }))}
                                                className="w-full pl-10 pr-4 py-2 bg-foreground/5 border border-card-border/50 rounded-xl text-foreground focus:outline-none focus:border-purple-500/50"
                                            />
                                        </div>
                                    </div>
                                    <div>
                                ...
                                <div className="flex gap-3 pt-2">
                                    <button
                                        type="button"
                                        onClick={resetForm}
                                        className="flex-1 py-2.5 bg-foreground/5 border border-card-border/50 rounded-xl text-muted hover:text-foreground hover:bg-foreground/10 transition-all"
                                    >
                                        Cancelar
                                    </button>
                                    <button
