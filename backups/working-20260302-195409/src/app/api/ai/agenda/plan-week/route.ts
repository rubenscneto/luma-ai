...
import { strictDateSchema, normalizeToTimestamptz, assertIntervalOrder, findAgendaCollision } from '@/lib/agendaValidation';
...
const planWeekInputSchema = z.object({
    ...
    start_date: strictDateSchema, // Sunday of the week
            ...
            try {
                ...
                for (const day of weekPlan.days) {
                    ...
                    day.blocks.forEach(b => {
                        const start = normalizeToTimestamptz(day.date, b.start_time, input.timezone);
                        const end = normalizeToTimestamptz(day.date, b.end_time, input.timezone);
                        try {
                            assertIntervalOrder(start, end);
                        } catch {
                            solverWarnings.push(`${day.date}: bloco inválido ignorado (${b.title})`);
                            return;
                        }
                        rawDayInputs.push({
                            title: b.title,
                    ...
                    const dayCollision = split.today.find((block, idx) => !!findAgendaCollision(block, split.today.slice(0, idx)));
                    if (dayCollision) {
                        solverWarnings.push(`${day.date}: conflito detectado antes do solver (${dayCollision.title})`);
                    }
